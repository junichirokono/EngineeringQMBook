...
%{\em Devices}: diodes, transistors, light emitters, photodetectors, modulators, É etc.
...
Take just a few minutes to break all the secret codes used by the worldÕs security agencies
Totally secure communication by encoding information in photons of light
ÔTeleportationÕ of quantum states of matter across large distances
...
Coherent states are very easily damaged by uncontrolled interactions with the environment Ð decoherence
...
\subsection{Nanomaterials}\index{nanomaterials}  \ce{C60}, carbon nanotubes, graphene, nanoshells, ...
 ...
 introduction to electrical conduction in solids; metals and insulators; hydrogen atom; periodic table; molecules; from bonds to bands; PauliÕs exclusion principle; particle in a box; and quantum wells, wires, and dots.
...
\section{Wave Equation and Wavefunction}

\subsection{de Broglie's Hypothesis}

Planck's light quanta, or photons, have each a photon energy
%
\begin{align}
E = h \nu
\label{Bohr}
\end{align}
%
where $h$ = 6.63 $\times$ 10$^{-34}$~(J$\cdot$s) is the Planck constant and $\nu$ is the frequency of light. On the other hand, according to Einstein's special relativity, any particle possess the following energy ($E$) -- momentum ($p$) relationship:
%
\begin{align}
...
\end{align}
%
where $c$ = 3.0 $\times$ 10$^8$~(m/s) is the speed of light and $m$ is the mass of the particle.  By applying Eq.~\eqref{Einstein} to photons, for which $m$ = 0,  we have
%
\begin{align}
E = c p.
\label{cp}
\end{align}
%
By equating Eq.~\eqref{Bohr} and Eq.~\eqref{cp}, and recalling $c = \nu \lambda$ (where $\lambda$ is the wavelength), we get
%
\begin{align}
\lambda = \frac{h}{p}.
\label{deBroglie}
\end{align}
%
In 1923, Louis de Broglie hypothesized that this relation should hold not only for photons but also \emph{all} matter; i.e., any particle moving with momentum $p$ should have a quantum mechanical wavelength, $h/p$.  In 1925, Davisson and Germer demonstrated \emph{diffraction} of an electron beam by a crystal of nickel; in 1989, Tonomura observed \emph{interference} of a beam of electrons using a Young's double-slit geometry.


\subsection{Schr\"{o}dinger's Wave Equation}\index{Schr\"odinger equation!time-independent}

Stimulated by de Brogie's work, Schr\"{o}dinger developed a wave equation in 1925, which is now known as the Schr\"odinger Equation.  He started from the classical wave equation,
%
\begin{align}
\nabla^2 \psi = - k^2 \psi,
\label{classic-wq}
\end{align}
%
where $k$ = 2$\pi$/$\lambda$ is the wavenumber and $\nabla^2$ is the Laplacian.  The de Broglie relation, Eq.~\eqref{deBroglie}, in terms of $k$ reads $p$ = $\hbar k$, where $\hbar$ = $h/2\pi$ is called the reduced Planck constant.  Therefore, we can rewrite $k^2$ as
%
\begin{align}
k^2 = \frac{p^2}{\hbar^2} = \frac{2m}{\hbar^2}(E-V),
\label{k^2}
\end{align}
%
where the following relation was used for the momentum ($p$), total energy ($E$), kinetic energy ($T$), and potential energy ($V$): $p^2/2m = T = E-V$.
%
By substituting Eq.~\eqref{k^2} into Eq.~\eqref{classic-wq}, we obtain the Schr\"odinger Equation:
%
\begin{align}
...
\end{align}
%
In 1926, Schr\"odinger solved this equation for $V(r) = -e^2/(4\pi \varepsilon_0 r)$ (i.e., an attractive Coulomb potential) to obtain wave functions and energies that successfully explained the emission spectrum of the hydrogen atom [Annalen der Physik \textbf{79}, 361 (1926)].

\subsection{Wavefunction}

Schr\"odinger was trying to develop a theory that explains atomic- and sub-atomic-scale phenomena entirely using the classical theory of waves.  As such, he was seeking an interpretation of the wavefunction, $\psi$, purely from a wave point of view, without success.  A correct interpretation of $\psi$, embracing the concept of wave-particle duality, was proposed by Max Born in 1926: ``The square of the amplitude of the wavefunction in some specific region of space is related to the probability of finding the associated quantum particle in that region of space.''  According to this interpretation, the wavefunction has to be properly normalized in the following sense:
%
\begin{align}
\int \psi(\vec{r}) \psi^*(\vec{r}) d^3\vec{r} = \int^{\infty}_{-\infty}\int^{\infty}_{-\infty}\int^{\infty}_{-\infty} |\psi(x,y,z)|^2 dxdydz = 1.
\label{noemalization}
\end{align}
...
Let us first consider a free particle, i.e., $V$ = 0 everywhere.  Then,
%
\begin{align}
...
\end{align}
%
where $A$ and $B$ are constants and $|\vec{k}|^2$ = $k^2$ = $2mE/\hbar^2$.

Next, let us consider the case when the potential energy can be separated into three separate spatial directions as
%
\begin{align}
V(\vec{r}) = V(x,y,z) = V_x(x) + V_y(y) + V_z(z).
\end{align}
%
In this case, the wavefunction can be written
%
\begin{align}
...
%
\begin{align}
-\frac{\hbar^2}{2m} \left( YZ\frac{d^2X}{dx^2} + XZ\frac{d^2Y}{dy^2} + XY\frac{d^2Z}{dz^2} \right ) +(V_x + V_y + V_z)XYZ = EXYZ,
\end{align}
%
or by dividing both sides by $XYZ$, we get
%
\begin{align}
\left ( -\frac{\hbar^2}{2m} \frac{d^2}{dx^2} + V_x \right ) X + \left ( -\frac{\hbar^2}{2m} \frac{d^2}{dy^2} + V_y \right ) Y + \left ( -\frac{\hbar^2}{2m} \frac{d^2}{dz^2} + V_z \right ) Z  = E,
\end{align}
%
which has to be satisfied for any values of $x$, $y$, and $z$.  This is possible only when the three terms on the left hand side are constants:
%
%
\begin{align}
\left ( -\frac{\hbar^2}{2m} \frac{d^2}{dx^2} + V_x \right ) X = E_x, \\
\left ( -\frac{\hbar^2}{2m} \frac{d^2}{dy^2} + V_y \right ) Y = E_y, \\
\left ( -\frac{\hbar^2}{2m} \frac{d^2}{dz^2} + V_z \right ) Z = E_z, \\
E_x + E_y + E_z = E.
\end{align}
%

Let us now consider a one-dimensional potential with an infinitely high potential barrier height:
%
\begin{align}
V(x) = 
\begin{cases}
...
\end{align}
%
Inside the quantum well, the Schroedinger equation reads:
%
%
\begin{align}
...
\end{align}
%
Boundary conditions: $\psi(\pm L)$ = 0.
%
%
\begin{align}
A e^{-ikL} + B e^{ikL} = 0,\\
...
%
\begin{align}
A (e^{2ikL} - e^{-2ikL}) = 2iA \sin(kL) = 0.
\end{align}
%
Therefore, 2$kL$ = $n\pi$, where $n$ = 1, 2, $...$, and
%
\begin{align}
E_n = \frac{\hbar^2 k^2}{2m} = \frac{\pi^2 \hbar^2}{8mL^2}n^2.
\end{align}
%
In a more general three-dimensional box with dimensions $L_x$ $times$ $L_y$ $\times$ $L_z$,
%
%
\begin{align}
E_{n_x,n_y,n_z} = \frac{\pi^2 \hbar^2}{8mL_x^2}n_x^2 + \frac{\pi^2 \hbar^2}{8mL_y^2}n_y^2 + \frac{\pi^2 \hbar^2}{8mL_z^2}n_z^2.
\end{align}
% 
